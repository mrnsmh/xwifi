...
prep_attack() {
  ...
  # Auto mode: use optimal defaults without user interaction
  if [ "$FLUXIONAutoCapture" ]; then
    echo "Auto-capture mode: using default settings" > $FLUXIONOutputDevice
    
    # Get available wireless interfaces
    interface_list_wireless
    if [ ${#InterfaceListWireless[@]} -gt 0 ]; then
      HandshakeSnooperJammerInterfaceOriginal="${InterfaceListWireless[0]}"
      if fluxion_allocate_interface "$HandshakeSnooperJammerInterfaceOriginal"; then
        HandshakeSnooperJammerInterface="$HandshakeSnooperJammerInterfaceOriginal"
      else
        echo "Failed to allocate interface in auto mode" > $FLUXIONOutputDevice
        return 1
      fi
    else
      echo "No wireless interfaces found for auto mode" > $FLUXIONOutputDevice
      return 1
    fi
    
    # Use mdk4 for deauth (most reliable)
    HandshakeSnooperDeauthenticatorIdentifier="$HandshakeSnooperMdk4MethodOption"
    # Use aircrack-ng for verification (always available)
    HandshakeSnooperVerifierIdentifier="aircrack-ng"
    # Check every 30 seconds
    HandshakeSnooperVerifierInterval=30
    # Non-blocking for faster capture
    HandshakeSnooperVerifierSynchronicity="non-blocking"
